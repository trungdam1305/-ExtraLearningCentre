...
<html>
    <head><title>Chỉnh sửa thông tin lớp học</title></head>
    <style>
        body {
...
String idLop = request.getParameter("ID_LopHoc");
        if (idLop == null) {
            idLop = (String) request.getAttribute("ID_LopHoc");
        }
        %>
...
if (idKhoi != null) {
    try {
        ...
        <form action="${pageContext.request.contextPath}/Class_ManagerClass" method="post" enctype="multipart/form-data">
            <input type="hidden" name="action" value="submitUpdateClass">
            <input type="hidden" name="ID_KhoaHoc" value="<%= idKhoaHoc %>" />
            <input type="hidden" name="ID_Khoi" value="<%= idKhoi %>" />
            <input type="hidden" name="ID_LopHoc" value="<%= idLop %>" />
            ...
            <input type="text" name="TenLopHoc" value="${lopHoc.tenLopHoc}" required />
            <br/>

            <label>Khóa học:</label>
            <p>Bạn đang chỉnh sửa thông tin lớp học cho khóa có ID: <%= idKhoaHoc %></p>
            <label>Khối học:</label>
            <p>Bạn đang chỉnh sửa thông tin lớp học cho khối: <%= tenLop  %></p>
            <label>Lớp học:</label>
            <p>Bạn đang chỉnh sửa thông tin lớp học có id: <%= idLop  %></p>
            <br/>
            ...
            <label>Ghi chú:</label>
            <input type="text" name="GhiChu" value="${lopHoc.ghiChu}" /><br/>

            <label>Trạng thái:</label>
            <select name="TrangThai" required>
                <option value="Active" ${lopHoc.trangThai == 'Active' ? 'selected' : ''}>Active</option>
                <option value="Inactive" ${lopHoc.trangThai == 'Inactive' ? 'selected' : ''}>Inactive</option>
            </select>
            <br/>
            <br/>
            <br/>
            ...
            <label>Sĩ số:</label>
            <input type="text" name="SiSo" min="10" value="${lopHoc.siSo}" required />
...
if ("1".equals(sucParam)) {
        %>
        <p style="color: green;">Chỉnh sửa lớp học thành công!</p>
        <%
        }
        %>
