...
<body class="bp-legacy page-template-default page page-id-2385 page-child parent-pageid-45 wp-embed-responsive theme-guru woocommerce-js tribe-js js"><div id="yith-wcwl-popup-message" style="display: none;"><div id="yith-wcwl-message"></div></div>
	  ...
	  <div class="content">
          <div class="container">
                  ...
                  <article id="post-2385" class="post-2385 page type-page status-publish hentry">
                      <div class="woocommerce columns-4">
                                <ul class="products columns-4" style="display: flex; flex-wrap: wrap; gap: 3px; padding: 0; list-style: none;">
                                    <!--Get data material from database-->
                                    <c:forEach var="taiLieu" items="${listTaiLieu}">
                                        <li class="post product type-product post-${taiLieu.ID_Material} status-publish instock has-post-thumbnail product-type-simple " data-danhmuc="${taiLieu.danhMuc}"
                                            style="flex: 0 0 calc(25% - 20px); box-sizing: border-box; display: flex; flex-direction: column; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; background-color: #fff; min-height: 300px;">
                                            ...
                                            <div class="product-wrapper" style="display: flex; flex-direction: column; height: 90%;">
                                                ...
                                                <div class="product-title" style="padding: 10px; flex-grow: 0;">
                                                    <h2 class="woocommerce-loop-product__title" style="font-size: 16px; min-height: 48px;">
                                                        ${taiLieu.tenTaiLieu}
                                                    </h2>
                                                </div>
                                                <!-- Description of Material -->
                                                <div class="product-details" style="padding: 0 10px 10px 10px; flex-grow: 1;">
                                                    <p style="font-size: 14px; max-height: 60px; overflow: hidden; text-overflow: ellipsis;">
                                                        ${taiLieu.danhMuc}
                                                    </p>
                                                    <p style="font-size: 13px; color: gray;">Loại: ${taiLieu.loaiTaiLieu}</p>
                                                    <p style="font-size: 13px; color: gray;">Giá Tiền: ${taiLieu.giaTien}</p>
              ...
              <section class="left-sidebar" id="secondary">
                  <div><h3>Tìm kiếm Tài Liệu</h3>
                      </div>
                    <aside id="woocommerce_product_search-4" class="widget woocommerce widget_product_search">
                        <!--Filter Material by Categories-->
                                <select id="danhMucFilter" class="course-filter-select" 
                                        style="margin-top: 10px; width: 100%; padding: 6px;">
                                    <option value="">-- Tất Cả Tài Liệu --</option>
                                    <c:forEach var="mh" items="${listDanhMuc}">
                                        <option value="${mh}">${mh}</option>
                                    </c:forEach>
                                </select>
                        <!--Search Categories by Name-->
                        <form role="search" method="post" class="woocommerce-product-search" action="HomePageMaterial">
                                <label class="screen-reader-text" for="woocommerce-product-search-field-0">Tìm Kiếm Tài Liệu:</label>
                                <input type="search" id="woocommerce-product-search-field-0" class="search-field" placeholder="Tìm Tài Liệu" value="" name="danhmuc">
                                <!-- Bộ lọc môn học -->
                                <button type="submit" value="Search">Tìm Tài Liệu</button>
                                <input type="hidden" name="post_type" value="product">

                        </form>
                    </aside>
                  <aside id="woocommerce_product_categories-7" class="widget woocommerce widget_product_categories">
                        <div class="widget-title"><h3>Phân loại Tài Liệu</h3><div class="title-sep"><span></span></div>
                        </div>
    ...
    <script>
document.getElementById("danhMucFilter").addEventListener("change", function () {
    const selected = this.value.toLowerCase();
    const items = document.querySelectorAll("li[data-danhmuc]");

    items.forEach(item => {
        const danhmuc = item.getAttribute("data-danhmuc").toLowerCase();
        if (selected === "" || danhmuc === selected) {
