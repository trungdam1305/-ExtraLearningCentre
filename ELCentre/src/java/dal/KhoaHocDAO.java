   ...
   public static int getTotalCoursesByTrangThai(String trangThai) {
    ...
    try {
        String sql = "SELECT ID_KhoaHoc, TenKhoaHoc, MoTa, ThoiGianBatDau, " +
                     "ThoiGianKetThuc, GhiChu, TrangThai, NgayTao, ID_Khoi " +
                     "FROM KhoaHoc WHERE TenKhoaHoc LIKE ? ORDER BY TenKhoaHoc ASC " +
    ...
    List<KhoaHoc> khoaHocList = new ArrayList<>();
    try {
        String sql = "SELECT * FROM KhoaHoc WHERE TrangThai = ? AND TenKhoaHoc LIKE ? " +
                     "ORDER BY ID_KhoaHoc OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        PreparedStatement statement = db.getConnection().prepareStatement(sql);
        statement.setString(1, trangThai);
        statement.setString(2, "%" + tenKhoaHoc + "%"); // tìm tên gần đúng
    ...
    DBContext db = DBContext.getInstance();
    try {
        String sql = "SELECT COUNT(*) FROM KhoaHoc WHERE TrangThai = ? AND TenKhoaHoc LIKE ?";
        PreparedStatement stmt = db.getConnection().prepareStatement(sql);
        stmt.setString(1, trangThai);
        stmt.setString(2, "%" + tenKhoaHoc + "%");
