...
//    }
    ...
    public static void main(String[] args) {
        int idHocSinh = 4; // Thay đổi ID học sinh phù hợp với DB của bạn

        List<LichHoc> dsLich = LichHocDAO.getLichHocByHocSinhId(idHocSinh);

        if (dsLich.isEmpty()) {
            System.out.println("❌ Không tìm thấy lịch học nào cho học sinh có ID = " + idHocSinh);
            return;
        }

        System.out.println("===== LỊCH HỌC CỦA HỌC SINH (ID = " + idHocSinh + ") =====");
        int index = 1;
        for (LichHoc lh : dsLich) {
            System.out.printf(" %02d. Ngày: %s | Slot: %s | Lớp: %s | Phòng: %s | Ghi chú: %s\n",
                    index++,
                    lh.getNgayHoc(),
                    lh.getSlotThoiGian(),
                    lh.getTenLopHoc(),
                    lh.getTenPhongHoc(),
                    lh.getGhiChu() != null ? lh.getGhiChu() : "Không có");
        }
    }
}